...
export default function ProfilePage() {
  ...
  async function handleAvatarUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFormData(f => ({ ...f, avatar: data.publicUrl }));
    } catch (err: any) {
        ...
        <div className="lg:col-span-1 space-y-8">
          <div className="bg-white rounded-[3rem] p-10 border border-on-surface/5 shadow-2xl shadow-primary/5 flex flex-col items-center text-center">
            <div className="relative group mb-8">
              <div className="w-40 h-40 rounded-[2.5rem] overflow-hidden ring-8 ring-slate-50 shadow-inner bg-slate-100 flex items-center justify-center relative">
                {formData.avatar ? (
                  <Image src={formData.avatar} alt="Avatar" fill className="object-cover transition-transform group-hover:scale-110" />
                ) : (
                  <User size={64} className="text-slate-300" />
                )}
              </div>
              <label className="absolute bottom-1 right-1 w-12 h-12 bg-primary text-white rounded-2xl border-4 border-white flex items-center justify-center shadow-lg hover:brightness-110 active:scale-90 transition-all cursor-pointer">
